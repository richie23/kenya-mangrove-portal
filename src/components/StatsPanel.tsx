...
// ── SVG CHART HELPERS ──────────────────────────────────────────────────────
function SvgColumnChart({ data, colors }: { data:{label:string,value:number}[], colors:string[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display:'block' }}>
      {data.map((d, i) => {
        const bh = Math.max(4, ((d.value / max) * (H - pad - 30)));
        const x = pad + i * ((W - pad * 2) / data.length) + 3;
        const y = H - pad - bh;
        return (
          <g key={d.label}>
            <rect x={x} y={y} width={bw} height={bh} rx="3" fill={colors[i % colors.length]}/>
            <text x={x + bw/2} y={y - 3} textAnchor="middle" fontSize="9" fill={colors[i % colors.length]} fontWeight="bold" fontFamily="Arial">{d.value}%</text>
            <text x={x + bw/2} y={H - 4} textAnchor="middle" fontSize="7" fill="#777" fontFamily="Arial">{d.label.split(' ')[0]}</text>
          </g>
        );
      })}
    </svg>
...
function SvgBarChart({ data, colors }: { data:{label:string,value:number}[], colors:string[] }) {
  const max = Math.max(...data.map(d => d.value));
  return (
    <svg viewBox={`0 0 300 ${data.length * 36 + 20}`} width="100%" style={{ display:'block' }}>
      {data.map((d, i) => {
        const w = Math.max(4, (d.value / max) * 220);
        return (
          <g key={d.label} transform={`translate(0,${i * 36 + 10})`}>
            <text x="0" y="12" fontSize="10" fill="#555" fontFamily="Arial">{d.label}</text>
            <rect x="0" y="16" width={w} height="14" rx="4" fill={colors[i % colors.length]}/>
            <text x={w + 4} y="27" fontSize="10" fill={colors[i % colors.length]} fontWeight="bold" fontFamily="Arial">{d.value}%</text>
          </g>
        );
      })}
    </svg>
...
function SvgPieChart({ data, colors, size=160 }: { data:{label:string,value:number}[], colors:string[], size?:number }) {
  const total = data.reduce((s, d) => s + d.value, 0);
  let angle = -Math.PI / 2;
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size}>
      {data.map((d, i) => {
        ...
        angle += slice;
        return <path key={d.label} d={path} fill={colors[i % colors.length]} stroke="white" strokeWidth="1.5"/>;
      })}
    </svg>
  );
}
function SvgDoughnut({ data, colors, size=160 }: { data:{label:string,value:number}[], colors:string[], size?:number }) {
  const total = data.reduce((s, d) => s + d.value, 0);
  let angle = -Math.PI / 2;
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size}>
      ...
      <text x={cx} y={cy+4} textAnchor="middle" fontSize="10" fontWeight="bold" fill="#085041" fontFamily="Arial">{data.length} spp</text>
    </svg>
  );
}
function ChartLegend({ data, colors }: { data:{label:string,value:number}[], colors:string[] }) {
  return (
    <div style={{ display:'flex', flexWrap:'wrap', gap:'4px', marginTop:'8px' }}>
      {data.map((d, i) => (
        <div key={d.label} style={{ display:'flex', alignItems:'center', gap:'4px', fontSize:'9px', color:'#555' }}>
          <div style={{ width:'8px', height:'8px', borderRadius:'2px', background:colors[i % colors.length], flexShrink:0 }}/>
          <span>{d.label.split(' ')[0]}: {d.value}%</span>
        </div>
      ))}
    </div>
  );
    ...
    <div style={{ padding:'0 2px' }}>
      ...
      {tab === 'species' && (
        <div>
          ...
          {COUNTIES.map((co) => {
            const spData = co.species.map(sp => ({ label: sp.n, value: sp.p }));
            const chartMap: Record<string,string> = { 'Lamu':'bar','Kilifi':'bar','Kwale':'column','Mombasa':'doughnut','Tana River':'pie' };
            const chartType = chartMap[co.name] || 'bar';
            const chartLabels: Record<string,string> = { 'bar':'Bar Chart','column':'Column Chart','doughnut':'Doughnut Chart','pie':'Pie Chart' };
            return (
            <div key={co.name} style={{ marginBottom:'16px', background:'white', borderRadius:'12px', padding:'14px', border:'1px solid #eee' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
                <p style={{ fontSize:'13px', fontWeight:'800', color:'#085041', margin:0 }}>{co.name} County</p>
                <div style={{ display:'flex', gap:'6px' }}>
                  <span style={{ background:'#E1F5EE', color:'#085041', fontSize:'10px', padding:'2px 8px', borderRadius:'8px', fontWeight:'700' }}>{chartLabels[chartType]}</span>
                  <span style={{ background:`${co.color}20`, color:co.color, fontSize:'10px', padding:'2px 8px', borderRadius:'8px', fontWeight:'700' }}>{co.area.toLocaleString()} ha</span>
                </div>
              </div>
              {chartType === 'bar' && <SvgBarChart data={spData} colors={PIE_COLORS}/>}
              {chartType === 'column' && <SvgColumnChart data={spData} colors={PIE_COLORS}/>}
              {chartType === 'doughnut' && <div style={{ display:'flex', justifyContent:'center', padding:'8px 0' }}><SvgDoughnut data={spData} colors={PIE_COLORS} size={150}/></div>}
              {chartType === 'pie' && <div style={{ display:'flex', justifyContent:'center', padding:'8px 0' }}><SvgPieChart data={spData} colors={PIE_COLORS} size={150}/></div>}
              <ChartLegend data={spData} colors={PIE_COLORS}/>
            </div>
            );
          })}
          {/* === DOWNLOADS === */}
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px', marginTop:'16px', paddingBottom:'8px' }}>
            <button onClick={() => {
              const rows: (string|number)[][] = [
                ['KENYA MANGROVE SPECIES REPORT'],
                ['Kenya National Mangrove Ecosystem Management Plan 2017-2027'],
                [`Generated: ${new Date().toLocaleDateString()}`],[''],
                ['NATIONAL SPECIES INVENTORY'],
                ['Scientific Name','Local Name','Estimated Area (ha)','Risk Status','Primary Use','Distribution'],
                ...SPECIES_DATA.map(s=>[s.name,s.local,s.area,s.risk,s.use,s.dominant]),
                [''],['SPECIES BY COUNTY'],
                ['County','Total Area (ha)','Species','Proportion (%)'],
                ...COUNTIES.flatMap(co=>co.species.map(sp=>[co.name,co.area,sp.n,sp.p])),
                [''],['SOURCE: GoK (2017). National Mangrove Ecosystem Management Plan. KFS, Nairobi.'],
              ];
              const csv=rows.map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n');
              const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));
              a.download='Kenya_Mangrove_Species_Report.csv';a.click();
            ...
            <button onClick={async () => {
              const{default:jsPDF}=await import('jspdf');
              const doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});
              const W=210,H=297,M=14,CW=W-M*2;let y=20;
              const sf=(h:string):{r:number,g:number,b:number}=>{try{const c=h.replace('#','').padEnd(6,'0');const s=(n:number)=>isNaN(n)?0:Math.min(255,Math.max(0,n));return{r:s(parseInt(c.slice(0,2),16)),g:s(parseInt(c.slice(2,4),16)),b:s(parseInt(c.slice(4,6),16))};}catch{return{r:8,g:80,b:65};}};
              const fill=(h:string)=>{const{r,g,b}=sf(h);doc.setFillColor(r,g,b);};
              const txt=(h:string)=>{const{r,g,b}=sf(h);doc.setTextColor(r,g,b);};
              fill('#0F6E56');doc.rect(0,0,W,55,'F');
              doc.setFontSize(11);doc.setFont('helvetica','bold');txt('#9FE1CB');
              doc.text('KENYA NATIONAL MANGROVE ECOSYSTEM MANAGEMENT PLAN 2017-2027',M,18);
              doc.setFontSize(18);txt('#FFFFFF');doc.text('MANGROVE SPECIES REPORT',M,32);
              doc.setFontSize(9);doc.setFont('helvetica','normal');txt('#9FE1CB');
              doc.text(`9 recorded species | KFS | KMFRI | ${new Date().toLocaleDateString('en-GB')}`,M,46);
              y=65;
              const chk=(n:number)=>{if(y+n>H-14){doc.addPage();y=20;}};
              const sec=(t:string,col='#0F6E56')=>{chk(14);y+=4;fill(col);doc.rect(M,y-5,CW,9,'F');doc.setFontSize(11);doc.setFont('helvetica','bold');txt('#FFFFFF');doc.text(t,M+3,y);y+=9;};
              const kv=(l:string,v:string,col='#0F6E56')=>{chk(8);const{r,g,b}=sf(col);doc.setFontSize(9);doc.setFont('helvetica','normal');txt('#555555');doc.text(l,M+2,y);doc.setFont('helvetica','bold');doc.setTextColor(r,g,b);doc.text(v,M+CW-2,y,{align:'right'});y+=7;doc.setDrawColor(220,220,220);doc.line(M,y-1,M+CW,y-1);};
              sec('National Species Inventory');
              SPECIES_DATA.forEach(s=>{
                chk(40);fill('#1D9E75');doc.rect(M,y,CW,8,'F');
                doc.setFontSize(10);doc.setFont('helvetica','bold');txt('#FFFFFF');
                doc.text(s.name+' ('+s.local+')',M+3,y+5.5);y+=12;
                kv('Risk Status',s.risk,s.rcolor);
                kv('Primary Use',s.use,'#085041');
                kv('Distribution',s.dominant,'#085041');
                kv('Estimated Area','~'+s.area.toLocaleString()+' ha','#0F6E56');
                const bw=Math.min(CW-4,(s.area/31000)*(CW-4));
                const{r,g,b}=sf(s.rcolor);doc.setFillColor(r,g,b);doc.rect(M+2,y,bw,5,'F');y+=10;
              });
              sec('Species by County');
              COUNTIES.forEach(co=>{
                chk(16);fill(co.color);doc.rect(M,y,CW,7,'F');
                doc.setFontSize(10);doc.setFont('helvetica','bold');txt('#FFFFFF');
                doc.text(co.name+' County - '+co.area.toLocaleString()+' ha',M+3,y+4.5);y+=10;
                co.species.forEach(sp=>{
                  chk(8);doc.setFontSize(9);doc.setFont('helvetica','normal');txt('#333333');
                  doc.text(sp.n,M+4,y);
                  const{r,g,b}=sf(co.color);doc.setTextColor(r,g,b);doc.setFont('helvetica','bold');
                  doc.text(sp.p+'%',M+CW-2,y,{align:'right'});
                  const bw=(sp.p/100)*(CW-8);fill(co.color);doc.rect(M+4,y+2,bw,3,'F');
                  y+=9;
                });
                y+=4;
              });
              const tot=(doc as any).internal.getNumberOfPages();fill('#0F6E56');
              for(let i=1;i<=tot;i++){doc.setPage(i);doc.rect(0,H-12,W,12,'F');doc.setFontSize(7);txt('#9FE1CB');doc.text(`Kenya Mangrove Portal | Species Report | Page ${i}/${tot}`,M,H-5);}
              doc.save('Kenya_Mangrove_Species_Report.pdf');
      ...
      {tab === 'totals' && (
        <div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px', marginTop:'16px', paddingBottom:'8px' }}>
            <button onClick={() => {
              const rows: (string|number)[][] = [
                ['KENYA MANGROVE NATIONAL TOTALS REPORT'],
                ['Kenya National Mangrove Ecosystem Management Plan 2017-2027'],
                [`Generated: ${new Date().toLocaleDateString()}`],[''],
                ['NATIONAL SUMMARY'],
                ['Metric','Value','Unit'],
                ['Total mangrove area',61271,'ha'],
                ['Degraded area',24585,'ha'],
                ['Healthy area',36686,'ha'],
                ['Degradation rate',40.1,'%'],
                ['Annual cover loss','~450','ha/yr'],
                ['Artisanal fishers','20,000+','people'],
                ['Fisheries dependence','70%','of commercial catch'],
                ['Ecosystem value',269448,'KES/ha/yr'],
                ['Mangrove species',9,'species'],
                ['Coastal counties',5,'counties'],
                ['Plan period','2017-2027',''],
                ['Plan budget',3800,'KES million'],
                [''],
                ['COUNTY BREAKDOWN'],
                ['County','Total Area (ha)','Degraded (ha)','Healthy (ha)','Degraded %','Healthy %','Carbon Min (tC/ha)','Carbon Max (tC/ha)','Fishers','Ecosystem Value (KES/ha/yr)','Restoration Target (ha/yr)','Stand Density (stems/ha)','Volume (m3/ha)','Note'],
                ...COUNTIES.map(c=>[c.name,c.area,Math.round(c.area*c.degraded/100),Math.round(c.area*c.healthy/100),c.degraded,c.healthy,c.carbon_min,c.carbon_max,c.fishers,c.ecosystem_val,c.restoration_target,c.density,c.volume,c.note]),
                ['KENYA TOTAL',61271,24585,36686,40.1,59.9,'500-1000','','20000+',269448,'','','',''],
                [''],
                ['MANAGEMENT BLOCKS BY COUNTY'],
                ['County','Block Name'],
                ...COUNTIES.flatMap(c=>c.blocks.map(b=>[c.name,b])),
                [''],
                ['THREATS BY COUNTY'],
                ...
                ['Programme','Budget (KES M)','Lead Agency'],
                ...PROGRAMMES.map(p=>[p.name,p.budget,p.lead]),
                ['TOTAL',PROGRAMMES.reduce((s,p)=>s+p.budget,0),'2017-2027'],
                [''],
                ['SOURCE: GoK (2017). National Mangrove Ecosystem Management Plan. KFS, Nairobi.'],
              ];
              const csv=rows.map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n');
              const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));
              a.download='Kenya_Mangrove_National_Totals_Report.csv';a.click();
            }} style={{ padding:'12px', background:'#085041', color:'white', border:'none', borderRadius:'10px', fontSize:'12px', fontWeight:'800', cursor:'pointer' }}>
              Download CSV
            </button>
            <button onClick={async () => {
              const{default:jsPDF}=await import('jspdf');
              ...
              const fill=(h:string)=>{const{r,g,b}=sf(h);doc.setFillColor(r,g,b);};
              const txt=(h:string)=>{const{r,g,b}=sf(h);doc.setTextColor(r,g,b);};
              fill('#085041');doc.rect(0,0,W,55,'F');
              doc.setFontSize(11);doc.setFont('helvetica','bold');txt('#9FE1CB');
              doc.text('KENYA NATIONAL MANGROVE ECOSYSTEM MANAGEMENT PLAN 2017-2027',M,18);
              doc.setFontSize(18);txt('#FFFFFF');doc.text('NATIONAL TOTALS REPORT',M,32);
              doc.setFontSize(9);doc.setFont('helvetica','normal');txt('#9FE1CB');
              doc.text(`5 Counties | 61,271 ha | KFS | KMFRI | ${new Date().toLocaleDateString('en-GB')}`,M,46);
              y=65;
              const chk=(n:number)=>{if(y+n>H-14){doc.addPage();y=20;}};
              const sec=(t:string,col='#085041')=>{chk(14);y+=4;fill(col);doc.rect(M,y-5,CW,9,'F');doc.setFontSize(11);doc.setFont('helvetica','bold');txt('#FFFFFF');doc.text(t,M+3,y);y+=9;};
              const kv=(l:string,v:string,col='#085041')=>{chk(8);const{r,g,b}=sf(col);doc.setFontSize(9);doc.setFont('helvetica','normal');txt('#555555');doc.text(l,M+2,y);doc.setFont('helvetica','bold');doc.setTextColor(r,g,b);doc.text(v,M+CW-2,y,{align:'right'});y+=7;doc.setDrawColor(220,220,220);doc.line(M,y-1,M+CW,y-1);};
              sec('National Statistics');
              kv('Total mangrove area','61,271 ha','#0F6E56');
              kv('Degraded area','24,585 ha (40.1%)','#E24B4A');
              kv('Healthy area','36,686 ha (59.9%)','#1D9E75');
              kv('Annual cover loss','~450 ha/yr','#E24B4A');
              kv('Artisanal fishers','20,000+','#185FA5');
              kv('Ecosystem value','KES 269,448/ha/yr','#085041');
              kv('Mangrove species','9 species','#0F6E56');
              kv('Plan budget','KES 3.8 billion (2017-2027)','#085041');
              sec('County Breakdown');
              COUNTIES.forEach(co=>{
                chk(28);
                fill(co.color);doc.rect(M,y,CW,7,'F');
                doc.setFontSize(10);doc.setFont('helvetica','bold');txt('#FFFFFF');
                doc.text(co.name+' County',M+3,y+4.5);y+=10;
                kv('Total area',co.area.toLocaleString()+' ha',co.color);
                kv('Degraded',Math.round(co.area*co.degraded/100).toLocaleString()+' ha ('+co.degraded+'%)','#E24B4A');
                kv('Healthy',Math.round(co.area*co.healthy/100).toLocaleString()+' ha ('+co.healthy+'%)','#1D9E75');
                const hw=(co.healthy/100)*CW;const dw=(co.degraded/100)*CW;
                fill('#1D9E75');doc.rect(M,y,hw,5,'F');fill('#E24B4A');doc.rect(M+hw,y,dw,5,'F');y+=10;
                kv('Carbon stocks',co.carbon_min+'-'+co.carbon_max+' tC/ha','#0F6E56');
                kv('Fishers',co.fishers.toLocaleString(),'#185FA5');
                kv('Restoration target',co.restoration_target.toLocaleString()+' ha/yr','#1D9E75');
                y+=4;
              });
              sec('Management Programmes');
              PROGRAMMES.forEach(p=>kv(p.name,'KES '+p.budget+'M | '+p.lead,'#0F6E56'));
              const tot=(doc as any).internal.getNumberOfPages();fill('#085041');
              for(let i=1;i<=tot;i++){doc.setPage(i);doc.rect(0,H-12,W,12,'F');doc.setFontSize(7);txt('#9FE1CB');doc.text(`Kenya Mangrove Portal | National Totals Report | Page ${i}/${tot}`,M,H-5);}
              doc.save('Kenya_Mangrove_National_Totals_Report.pdf');
            }} style={{ padding:'12px', background:'#185FA5', color:'white', border:'none', borderRadius:'10px', fontSize:'12px', fontWeight:'800', cursor:'pointer' }}>
              Download PDF
            </button>
          </div>
  ...
  return (
    <div style={{ padding:'0 2px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px', marginTop:'16px', paddingBottom:'8px' }}>
        <button onClick={() => {
          const rows: (string|number)[][] = [
            ['KENYA MANGROVE DEGRADATION FIELD SURVEY 2023-2024'],
            ['Blue Carbon Project + IUCN | Kenya Forest Service | KMFRI'],
            [`Generated: ${new Date().toLocaleDateString()}`],[''],
            ['SURVEY OVERVIEW'],['Metric','Value'],
            ['Total survey sites',192],['Counties covered','5 (all coastal counties)'],
            ['Survey period','2023-2024'],['Funding','Blue Carbon Project + IUCN'],
            ['Total degraded area found','1,172 ha'],['Sites under rehabilitation',42],['Successfully restored',25],
            [''],['COUNTY RESULTS'],
            ['County','Survey Sites','Degraded Area (ha)','% of County Area','Main Cause'],
            ['Lamu',73,64,'0.2%','Illegal harvesting'],
            ['Kilifi',37,656,'7.7%','Climate change'],
            ['Kwale',68,277,'3.3%','Rice farm conversion'],
            ['Mombasa',6,153,'4.1%','Oil pollution'],
            ['Tana River',8,22,'0.7%','Sea level rise'],
            ['TOTAL',192,1172,'1.9% of national total','Multiple causes'],
            [''],['DEGRADATION STATUS'],['Status','Sites','Percentage'],
            ['Degraded (active)',125,'65.1%'],['Under rehabilitation',42,'21.9%'],['Successfully restored',25,'13.0%'],
            [''],['MAIN CAUSES (RANKED)'],['Rank','Cause'],
            [1,'Illegal harvesting and overexploitation of poles'],
            [2,'Conversion to aquaculture and rice farms'],
            [3,'Sedimentation and river damming'],
            [4,'Oil pollution and urban encroachment'],
            [5,'Climate change and sea level rise'],
            [6,'Settlement encroachment'],[''],
            ['DATA SOURCE: Blue Carbon Project / IUCN Field Survey 2023-2024 | KFS | KMFRI'],
          ];
          const csv=rows.map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n');
          const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));
          a.download='Kenya_Mangrove_Degradation_Survey_2023-2024.csv';a.click();
        }} style={{ padding:'12px', background:'#A32D2D', color:'white', border:'none', borderRadius:'10px', fontSize:'12px', fontWeight:'800', cursor:'pointer' }}>
          Download CSV
        </button>
        <button onClick={async () => {
          const{default:jsPDF}=await import('jspdf');
          const doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});
          const W=210,H=297,M=14,CW=W-M*2;let y=20;
          const sf=(h:string):{r:number,g:number,b:number}=>{try{const c=h.replace('#','').padEnd(6,'0');const s=(n:number)=>isNaN(n)?0:Math.min(255,Math.max(0,n));return{r:s(parseInt(c.slice(0,2),16)),g:s(parseInt(c.slice(2,4),16)),b:s(parseInt(c.slice(4,6),16))};}catch{return{r:162,g:45,b:45};}};
          const fill=(h:string)=>{const{r,g,b}=sf(h);doc.setFillColor(r,g,b);};
          const txt=(h:string)=>{const{r,g,b}=sf(h);doc.setTextColor(r,g,b);};
          fill('#A32D2D');doc.rect(0,0,W,55,'F');
          doc.setFontSize(11);doc.setFont('helvetica','bold');txt('#FFCCCC');
          doc.text('KENYA MANGROVE DEGRADATION FIELD SURVEY 2023-2024',M,18);
          doc.setFontSize(16);txt('#FFFFFF');doc.text('DEGRADATION SURVEY REPORT',M,32);
          doc.setFontSize(9);doc.setFont('helvetica','normal');txt('#FFCCCC');
          doc.text('192 sites | 5 counties | Blue Carbon Project + IUCN | KFS | KMFRI',M,46);
          y=65;
          const chk=(n:number)=>{if(y+n>H-14){doc.addPage();y=20;}};
          const sec=(t:string,col='#A32D2D')=>{chk(14);y+=4;fill(col);doc.rect(M,y-5,CW,9,'F');doc.setFontSize(11);doc.setFont('helvetica','bold');txt('#FFFFFF');doc.text(t,M+3,y);y+=9;};
          const kv=(l:string,v:string,col='#A32D2D')=>{chk(8);const{r,g,b}=sf(col);doc.setFontSize(9);doc.setFont('helvetica','normal');txt('#555555');doc.text(l,M+2,y);doc.setFont('helvetica','bold');doc.setTextColor(r,g,b);doc.text(v,M+CW-2,y,{align:'right'});y+=7;doc.setDrawColor(220,220,220);doc.line(M,y-1,M+CW,y-1);};
          sec('Survey Overview');
          kv('Total survey sites','192','#A32D2D');kv('Counties covered','5 (all coastal)','#085041');
          kv('Survey period','2023-2024','#085041');kv('Funding','Blue Carbon Project + IUCN','#185FA5');
          kv('Total degraded area','1,172 ha','#A32D2D');kv('Under rehabilitation','42 sites','#BA7517');kv('Restored','25 sites','#1D9E75');
          sec('County Results');
          [['Lamu',73,64,'0.2%','#1D9E75'],['Kilifi',37,656,'7.7%','#E24B4A'],['Kwale',68,277,'3.3%','#BA7517'],['Mombasa',6,153,'4.1%','#E24B4A'],['Tana River',8,22,'0.7%','#1D9E75']].forEach(([co,sites,area,pct,c])=>{
            kv(String(co)+' - '+sites+' sites',String(area)+' ha ('+pct+')',String(c));
          });
          sec('Main Degradation Causes');
          ['Illegal harvesting and overexploitation','Aquaculture and rice farm conversion','Sedimentation and river damming','Oil pollution and urban encroachment','Climate change and sea level rise','Settlement encroachment'].forEach((c,i)=>{chk(8);doc.setFontSize(9);doc.setFont('helvetica','normal');txt('#333333');doc.text((i+1)+'. '+c,M+4,y);y+=8;});
          const tot=(doc as any).internal.getNumberOfPages();fill('#A32D2D');
          for(let i=1;i<=tot;i++){doc.setPage(i);doc.rect(0,H-12,W,12,'F');doc.setFontSize(7);txt('#FFCCCC');doc.text('Kenya Mangrove Portal | Degradation Survey 2023-2024 | Page '+i+'/'+tot,M,H-5);}
          doc.save('Kenya_Mangrove_Degradation_Survey_2023-2024.pdf');
        }} style={{ padding:'12px', background:'#185FA5', color:'white', border:'none', borderRadius:'10px', fontSize:'12px', fontWeight:'800', cursor:'pointer' }}>
          Download PDF
        </button>
      </div>
              ...
              <div>
                ...
                <div style={{ flex:1, overflowY:'auto', padding:'4px 16px 12px' }}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px', padding:'0 0 16px', marginTop:'8px' }}>
                    <button onClick={() => {
                        const dHa=Math.round(rc.area*rc.degraded/100);const hHa=Math.round(rc.area*rc.healthy/100);
                        const allRows:(string|number)[][]=[
                          ['KENYA NATIONAL MANGROVE ECOSYSTEM MANAGEMENT PLAN 2017-2027'],
                          [rc.name+' County - Full Dataset Report'],
                          ['Generated: '+new Date().toLocaleDateString()],[''],
                          ['AREA DATA'],['Parameter','Value','Unit'],
                          ['Total mangrove area',rc.area,'ha'],['Degraded area',dHa,'ha'],['Healthy area',hHa,'ha'],
                          ['Degradation rate',rc.degraded,'%'],['National share',+((rc.area/61271)*100).toFixed(1),'%'],
                          ['Stand density',rc.density,'stems/ha'],['Volume',rc.volume,'m3/ha'],
                          ['Restoration target',rc.restoration_target,'ha/yr'],[''],
                          ['MANAGEMENT BLOCKS'],['Block Name'],...rc.blocks.map((b:string)=>[b]),[''],
                          ['SPECIES COMPOSITION'],['Species','Proportion (%)'],...rc.species.map((s:{n:string,p:number})=>[s.n,s.p]),[''],
                          ['BLUE CARBON'],['Parameter','Value','Unit'],
                          ['Carbon min',rc.carbon_min,'tC/ha'],['Carbon max',rc.carbon_max,'tC/ha'],
                          ['Total C min',rc.area*rc.carbon_min,'tC'],['Total C max',rc.area*rc.carbon_max,'tC'],[''],
                          ['ECOSYSTEM SERVICES'],['Service','KES/ha/yr','County Total KES/yr'],
                          ...ECOSYSTEM_SERVICES.map((e:{service:string,value:number})=>[e.service,e.value,rc.area*e.value]),[''],
                          ['THREATS'],['Rank','Threat'],...rc.threats.map((t:string,i:number)=>[i+1,t]),[''],
                          ['NOTES'],[rc.note],
                          ['SOURCE: GoK (2017). National Mangrove Ecosystem Management Plan. KFS, Nairobi.'],
                        ];
                        const csv=allRows.map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n');
                        const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));
                        a.download='Kenya_Mangrove_'+rc.name+'_Full_Report.csv';a.click();
                      }}
                      style={{ padding:'12px', background:'#0F6E56', color:'white', border:'none', borderRadius:'10px', fontSize:'12px', fontWeight:'800', cursor:'pointer' }}>
                      Download CSV
                    </button>
